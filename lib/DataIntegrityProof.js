...
const VM_CONTEXT_URL = 'https://w3id.org/security/multikey/v1';
...
export class DataIntegrityProof extends LinkedDataProof {
  ...
  async assertVerificationMethod({verificationMethod}) {
    ...
    if(!_includesContext({
      document: verificationMethod, contextUrl: VM_CONTEXT_URL
    })) {
      ...
      throw new TypeError(
        `The verification method (key) must contain "${VM_CONTEXT_URL}" ` +
        `context.`
      );
    }
