...
export class DataIntegrityProof extends LinkedDataProof {
  constructor({signer, date, cryptosuite} = {}) {
    ...
    const {
      canonize, createVerifier, name, requiredAlgorithm, derive
    } = cryptosuite;
  ...
  async verifySignature({verifyData, verificationMethod, proof}) {
    ...
    const multibaseHeader = proofValue.slice(1);
    // FIXME: add support for base64url as well
    if(multibaseHeader !== MULTIBASE_BASE58BTC_HEADER) {
      throw new Error('Only base58btc multibase encoding is supported.');
    }
    const signatureBytes = base58btc.decode(proofValue.slice(1));
   ...
   */
  async createVerifyData({document, proof, proofSet, documentLoader}) {
    // use custom cryptosuite `createVerifyData` if available
    if(this._cryptosuite.createVerifyData) {
      return this._cryptosuite.createVerifyData({
        cryptosuite: this._cryptosuite,
        document, proof, proofSet, documentLoader,
